...
def events_nations_over_time(df):
...
def most_successful(df,sport):
    ...
    x.rename(columns={'count':'Medals','region':'Country'})
    return x


def yearwise_medal_tally(temp_df,country):
...
def country_event_heatmap(df, country):
    ...
    # Create a pivot table of medal count by sport and year
...
def most_successful_countrywise(df,country):
    ...
    x = temp_df['Name'].value_counts().reset_index().merge(df,left_on='Name',right_on='Name',how='left')[['Name','count','Sport','region']].drop_duplicates(['Name']).reset_index().drop('index',axis=1) # x.rename(columns={'count':'Medals','region':'Country'})
    return x
